...
module m_lso_filter
    ...
    public :: s_initialize_lso_filter_module, &
              s_apply_lso_filter,             &
              s_finalize_lso_filter_module
            ...
            do ipass = 1, lso_n_passes_x
                ...
                do l = 0, p
                    do k = 0, n
                        do j = 0, m
                            lso_tmp(j, k, l) = &
                                c0*real(q_cons_vf(i)%sf(j,   k, l), wp) + &
                                c1*(real(q_cons_vf(i)%sf(j - 1, k, l), wp) + &
                                    real(q_cons_vf(i)%sf(j + 1, k, l), wp)) + &
                                c2*(real(q_cons_vf(i)%sf(j - 2, k, l), wp) + &
                                    real(q_cons_vf(i)%sf(j + 2, k, l), wp)) + &
                                c3*(real(q_cons_vf(i)%sf(j - 3, k, l), wp) + &
                                    real(q_cons_vf(i)%sf(j + 3, k, l), wp)) + &
                                c4*(real(q_cons_vf(i)%sf(j - 4, k, l), wp) + &
                                    real(q_cons_vf(i)%sf(j + 4, k, l), wp))
                        end do
                    end do
                end do
                ...
                do l = 0, p
                    do k = 0, n
                        do j = 0, m
                            lso_tmp(j, k, l) = &
                                c0*real(q_cons_vf(i)%sf(j, k,   l), wp) + &
                                c1*(real(q_cons_vf(i)%sf(j, k - 1, l), wp) + &
                                    real(q_cons_vf(i)%sf(j, k + 1, l), wp)) + &
                                c2*(real(q_cons_vf(i)%sf(j, k - 2, l), wp) + &
                                    real(q_cons_vf(i)%sf(j, k + 2, l), wp)) + &
                                c3*(real(q_cons_vf(i)%sf(j, k - 3, l), wp) + &
                                    real(q_cons_vf(i)%sf(j, k + 3, l), wp)) + &
                                c4*(real(q_cons_vf(i)%sf(j, k - 4, l), wp) + &
                                    real(q_cons_vf(i)%sf(j, k + 4, l), wp))
                        end do
                    end do
                end do
                $:END_GPU_PARALLEL_LOOP()
                ...
                do l = 0, p
                    do k = 0, n
                        do j = 0, m
                            lso_tmp(j, k, l) = &
                                c0*real(q_cons_vf(i)%sf(j, k, l),   wp) + &
                                c1*(real(q_cons_vf(i)%sf(j, k, l - 1), wp) + &
                                    real(q_cons_vf(i)%sf(j, k, l + 1), wp)) + &
                                c2*(real(q_cons_vf(i)%sf(j, k, l - 2), wp) + &
                                    real(q_cons_vf(i)%sf(j, k, l + 2), wp)) + &
                                c3*(real(q_cons_vf(i)%sf(j, k, l - 3), wp) + &
                                    real(q_cons_vf(i)%sf(j, k, l + 3), wp)) + &
                                c4*(real(q_cons_vf(i)%sf(j, k, l - 4), wp) + &
                                    real(q_cons_vf(i)%sf(j, k, l + 4), wp))
                        end do
                    end do
                end do
                $:END_GPU_PARALLEL_LOOP()
